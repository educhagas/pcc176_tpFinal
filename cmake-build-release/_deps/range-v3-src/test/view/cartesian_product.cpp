...
void test_bug_823() {
    ...
    CPP_assert(!(ranges::random_access_range<
    const decltype(three)> && ranges::view_<
    const decltype(three)>));
    ...
    CPP_assert(!(ranges::random_access_range<
    const decltype(prod)> && ranges::view_<
    const decltype(prod)>));
    ...
    CPP_assert(!(ranges::random_access_range<
    const decltype(twoD)> && ranges::view_<
    const decltype(twoD)>));
...
void test_bug_1269() {
    ...
    constexpr
    std::size_t N = ranges::size(data0) * ranges::size(data1) *
    ...
    CPP_assert(ranges::sized_sentinel_for < ranges::sentinel_t < decltype(rng) > ,
               ranges::iterator_t < decltype(rng) >> );
