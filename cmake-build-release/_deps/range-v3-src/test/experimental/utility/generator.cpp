...
template<typename T>
constexpr bool is_copy_constructible_or_ref()

noexcept
{
return std::is_reference<T>::value ||(bool)
ranges::copy_constructible<T>;
}
...
private:
    ...
    static generator_for<V> impl(V v) {
        if (RANGES_CONSTEXPR_IF(ranges::sized_range < V > ))
            co_await
        static_cast<ranges::experimental::generator_size>((std::size_t) ranges::distance(v));
...
public:
    ...
    !meta::is<ranges::uncvref_t < Rng>, ranges::experimental::sized_generator>
    ::value &&
            ranges::input_range<Rng>
    &&
    is_copy_constructible_or_ref<ranges::range_reference_t < Rng>>()))
    generator_for<ranges::views::all_t < Rng>> operator()(
    Rng &&rng
    ) const
    {
    ...
    operator()(ranges::experimental::generator <R, V> g) const
    ...
    operator()(ranges::experimental::sized_generator <R, V> g) const
...
ranges::experimental::sized_generator<int> g(int const n) {
    co_await
    static_cast<ranges::experimental::generator_size>((std::size_t)(n > 0 ? n : 0));
    for (int i = 0; i < n; ++i)
        co_yield i;
}
...
!ranges::sized_sentinel_for <S, T> && !(
ranges::integral <T> &&ranges::integral<S>
)))
...
CPP_template(class T, class S)(
        requires ranges::sized_sentinel_for <S, T>
...
ranges::experimental::sized_generator <T> iota_generator(T t, S const s) {
    co_await
    static_cast<ranges::experimental::generator_size>((std::size_t)(s - t));
    for (; t != s; ++t)
        co_yield t;
}
...
{
if(
RANGES_CONSTEXPR_IF(ranges::sized_range<V>)
)
co_await static_cast
<ranges::experimental::generator_size>((std::size_t)
ranges::distance(view)
);
