    ...
    namespace detail {
        ...
        namespace coroutine_owner_ {
            ...
            void swap(experimental::coroutine_owner<Promise> & x,
                      experimental::coroutine_owner<Promise> & y)
            noexcept {
            x.
            swap(y);
        }
...
namespace experimental {
    ...
    constexpr explicit coroutine_owner(base_t coro)

    noexcept
    :
    base_t(coro)
            {}
    coroutine_owner(coroutine_owner
    && that) noexcept
    :
...
coroutine_owner &operator=(coroutine_owner
that) noexcept
{
swap(that);
return *this;
...
void swap(coroutine_owner & that)
noexcept
{
...
namespace detail {
    ...
    struct generator_promise : experimental::enable_coroutine_owner {
        ...
        generator_promise *get_return_object()
        ...
        RANGES_COROUTINES_NS::suspend_always initial_suspend() const
        ...
        RANGES_COROUTINES_NS::suspend_always final_suspend() const
        ...
        AND
                std::is_assignable<semiregular_box_t < Reference>
        &, Arg>::value) //
        RANGES_COROUTINES_NS::suspend_always yield_value(Arg &&arg)

        noexcept(
                std::is_nothrow_assignable<semiregular_box_t < Reference>
        &, Arg>::value)
        {
        ...
        RANGES_COROUTINES_NS::suspend_never await_transform(
                experimental::generator_size) const
    ...
    struct sized_generator_promise : generator_promise<Reference> {
        sized_generator_promise *get_return_object()
        ...
        RANGES_COROUTINES_NS::suspend_never initial_suspend() const
        ...
        RANGES_COROUTINES_NS::suspend_always await_transform(
                experimental::generator_size size)
        ...
        experimental::generator_size_t size() const
...
namespace experimental {
    ...
    private:
        ...
        struct cursor {
            ...
            constexpr explicit cursor(handle coro)
