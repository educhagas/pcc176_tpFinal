...
namespace ranges {
    ...
    subrange <I1> RANGES_FUNC(search)
    (
    I1 begin1,
            S1
    end1,
...
if(
RANGES_CONSTEXPR_IF(sized_sentinel_for<S1, I1>
&&
sized_sentinel_for <S2, I2>))
return

detail::search_sized_impl(std::move(begin1),
        std::move(end1),
...
detail::search_impl(std::move(begin1),
        std::move(end1),
        std::move(begin2),
        std::move(end2),
        pred,
...
{
    if (empty(rng2))
        return subrange < iterator_t<Rng1>>
    {begin(rng1), begin(rng1)};
