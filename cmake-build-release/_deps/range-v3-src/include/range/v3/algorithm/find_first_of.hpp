...
namespace ranges {
    ...
    /// \pre
    requires input_iterator<I0>
    AND sentinel_for<S0, I0>
    AND
            forward_iterator<I1>
    ...
    ) //
{
    ...
    ++tmp)
    if(
    invoke(pred, invoke(proj0, *begin0), invoke(proj1, *tmp)
    ))
    return
    begin0;
    return
...
/// \pre
requires input_range<Rng0>
AND forward_range<Rng1>
AND
        indirect_relation<R,
        projected < iterator_t < Rng0>, P0
>,
projected <iterator_t<Rng1>, P1>>)
constexpr borrowed_iterator_t<Rng0>

RANGES_FUNC (find_first_of)(
