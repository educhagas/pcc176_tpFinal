...
namespace ranges {
    ...
    subrange <I> RANGES_FUNC(search_n)
    (
    I first,
            S
    last,
...
if(
RANGES_CONSTEXPR_IF(sized_sentinel_for<S, I>)
)
return

detail::search_n_sized_impl(std::move(first),
        std::move(last),
        distance(first, last),
        cnt,
        val,
...
AND
        indirectly_comparable<iterator_t < Rng>, V
const *, C, P>)
