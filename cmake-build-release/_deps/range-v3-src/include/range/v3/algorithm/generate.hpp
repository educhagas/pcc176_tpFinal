...
namespace ranges {
    ...
    AND output_iterator<O, invoke_result_t < F & >>
    AND
            sentinel_for<S, O>
    )
    generate_result<O, F> RANGES_FUNC(generate)
    (
    O first, S
    last,
    F fun
    ) //
{
    ...
    *
    first = invoke(fun);
    return {
    detail::move(first), detail::move(fun)
};
}
...
/// \pre
requires invocable<F &>
AND output_range<Rng, invoke_result_t < F & >>
)
