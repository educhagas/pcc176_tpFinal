...
namespace ranges {
    ...
    return
    detail::is_heap_until_n(std::move(first),
            distance(first, last),
            std::move(pred),
            std::move(proj)
    );
...
{
return
detail::is_heap_n(std::move(first),
        distance(first, last),
        std::move(pred),
        std::move(proj)
);
...
/// \pre
requires random_access_range<Rng>
AND sortable<iterator_t < Rng>, C, P
>)
borrowed_iterator_t <Rng> //
RANGES_FUNC(push_heap)
(
Rng &&rng, C
pred = C{}, P
proj = P{}
