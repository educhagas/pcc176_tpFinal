...
namespace ranges {
    ...
    O RANGES_FUNC(partial_sort_copy)
    (
    I first,
            SI
    last,
    ...
    ) //
{
    ...
    *
    ...
    auto &&x = *first;
    if(
    invoke(pred, invoke(in_proj, x), invoke(out_proj, *out_begin)
    )) {
    *
    out_begin = (decltype(x) && )
    x;
...
/// \pre
requires input_range<InRng>
AND random_access_range<OutRng>
AND
