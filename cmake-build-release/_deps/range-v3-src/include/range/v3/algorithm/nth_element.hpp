...
namespace ranges {
    ...
    namespace detail {
        ...
        AND indirect_relation<C, projected < I, P>>
        )
    ...
    ) //
{
    ...
    case 2:
    if(
    invoke(pred, invoke(proj, *--last), invoke(proj, *first)
    ))
...
if(!
invoke(pred, invoke(proj, *i), invoke(proj, *m)
)) // if *first == *m
{
...
invoke(
        ...
        proj,
        *--j)
)) // we need a guard if *first == *(last-1)
{
while(true)
{
if(i == j)
return
end_orig; // [first, last) all equivalent
// elements
if(
invoke(
...
if(
invoke(pred, invoke(proj, *j), invoke(proj, *m)
))
{
ranges::iter_swap(i, j
);
...
if(i !=
m &&invoke(pred, invoke(proj, *m), invoke(proj, *i))
)
{
