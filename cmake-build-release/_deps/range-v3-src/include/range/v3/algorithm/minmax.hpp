...
namespace ranges {
    ...
    /// \pre
    requires indirect_strict_weak_order<C, projected
    <T const *, P>>)
    constexpr minmax_result<T const &>
    ...
    *>)
        ...
        if (++first != last) {
            {
                ...
                if (invoke(pred, invoke(proj, tmp), invoke(proj, result.min)))
                    result.min = (decltype(tmp) && )
                tmp;
                else
                result.max = (decltype(tmp) && )
                tmp;
            }
            while (++first != last) {
    ...
    requires copyable<T>
    AND
            indirect_strict_weak_order<C, projected
    <T const *, P>>)
