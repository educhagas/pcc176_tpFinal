...
namespace ranges {
    ...
    namespace detail {
        ...
        sample_result<I, O> sample_sized_impl(I first,
            ...
            if (pop_size > 0 && sample_size > 0) {
                ...
                using param_t = typename decltype(dist)
                ::param_type;
    ...
    sample_result<I, O> RANGES_FUNC(sample)
    (
    I first,
            S
    last,
    ...
    if(
    RANGES_CONSTEXPR_IF(forward_iterator<I>
    || sized_sentinel_for <S, I>)) //
{
    auto const k = distance(first, last);
    return
    detail::sample_sized_impl(std::move(first),
            std::move(last),
            k,
            std::move(out),
            n,
...
|| sized_range <ORng>)
AND
(random_access_iterator<iterator_t < ORng>>
|| forward_iterator <I> ||
sized_sentinel_for <S, I>))
...
AND
(random_access_iterator<O>
|| forward_range <Rng> || sized_range <Rng>))
