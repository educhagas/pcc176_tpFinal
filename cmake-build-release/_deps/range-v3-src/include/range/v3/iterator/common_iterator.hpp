...
namespace ranges {
    ...
    namespace detail {
        ...
        template<typename I, typename S>
        variant <I, S> const &cidata(common_iterator < I, S >
        const & that) {
        return that.
        data_;
    }
...
private:
    ...
    template<typename T>
    static T *operator_arrow_(T *p, int)
    ...
    static J operator_arrow_(J const &j, int)

    noexcept (noexcept(J(j))) {
        return j;
    }
    ...
    static meta::_t <std::add_pointer<R>> operator_arrow_(J const &j, long)
...
public:
    ...
    auto operator->() const //
    noexcept (
    noexcept(common_iterator::operator_arrow_(std::declval<I const &>(),

    42)))
    ->

    decltype (common_iterator::operator_arrow_(std::declval<J const &>(),
