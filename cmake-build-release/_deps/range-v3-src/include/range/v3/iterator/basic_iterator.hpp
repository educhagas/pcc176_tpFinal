...
namespace ranges {
    ...
    struct basic_mixin : private box<T> {
        ...
        explicit CPP_ctor (basic_mixin)(T &&t)(          //
        ...
        explicit CPP_ctor (basic_mixin)(T const &t)(     //
    ...
    namespace detail {
        ...
        decltype(range_access::read(std::declval < Cur
        const &>()));
        ...
        decltype(detail::cursor_move(std::declval < Cur
        const &>(), 42));
        ...
        struct proxy_reference_conversion {
            ...
            noexcept (noexcept(Head(Head(std::declval<Derived const &>()
        ...
        public:
        constexpr reference_t_

        read_() const

        noexcept(
                noexcept(reference_t_(range_access::read(std::declval < Cur
        const &>())))) {
        return
        range_access::read(*cur_);
    }
    ...
    basic_proxy_reference_(basic_proxy_reference_
    const &) = default;
    ...
    constexpr basic_proxy_reference_(
            basic_proxy_reference < OtherCur >
    const & that) noexcept
    :
    cur_(that
    .cur_) {
...
constexpr explicit basic_proxy_reference_(Cur &cur)
...
constexpr basic_proxy_reference_
& //
operator=(basic_proxy_reference<OtherCur>
&& that)
{
return *this =
that;
...
/// \pre
requires readable_cursor<Cur>
AND equality_comparable<cursor_value_t < Cur>>
)
constexpr bool operator==(basic_proxy_reference_ < Cur, IsReadable >
const & x,
...
constexpr bool operator==(cursor_value_t < Cur >
const & x,
...
!range_access::single_pass_t<uncvref_t < C>>
::value ||
(
...
concept (cpp17_forward_cursor_)(C),
        std::is_reference<cursor_reference_t < C>>
...
decltype(range_access::arrow(std::declval < Cur
const &>()));
...
template(typename OtherCur)(
/// \pre
requires (!same_as <OtherCur, Cur>)
AND convertible_to<OtherCur, Cur>
AND
...
&
operator=(basic_iterator<OtherCur>
that)
{
...
noexcept(noexcept(range_access::read(std::declval < Cur
const &>())))
->
...
constexpr detail::cursor_arrow_t<C>

operator->() const

noexcept(noexcept(range_access::arrow(std::declval < C
const &>())))
{
return

range_access::arrow (pos());
...
constexpr std::add_pointer_t<const_reference_t>
...
private:
constexpr basic_iterator
post_increment_(std::false_type,
int)
{
...
{
range_access::advance(pos(), (difference_type)
...
friend constexpr auto iter_move(basic_iterator const &it)

noexcept(
        noexcept(range_access::move(std::declval < C
const &>())))
->
CPP_broken_friend_ret(
        decltype(range_access::move(std::declval < C
const &>())))(
/// \pre
requires same_as<C, Cur>
&& detail::input_cursor <Cur>)
{
return
range_access::move(it
.
...
constexpr bool operator==(basic_iterator < Cur >
const & left,
...
range_access::equal(range_access::pos(left), range_access::pos(right)
...
range_access::equal(range_access::pos(left), right
...
operator-(basic_iterator<Cur2>
const & left,
basic_iterator<Cur> const &right
)
{
return
range_access::distance_to(range_access::pos(right),
...
{
return
range_access::distance_to(range_access::pos(right), left
);
}
template(typename Cur, typename S)(
/// \pre
...
constexpr bool operator<(basic_iterator < Left >
const & left,
...
struct get_cursor_fn {
    ...
    operator()(basic_iterator<Cur> &it) const
    ...
    operator()(basic_iterator<Cur> const &it) const
