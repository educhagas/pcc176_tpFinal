...
namespace detail {
    ...
    namespace optional_adl {
        ...
        private:
            ...
            constexpr optional_storage(tag, std::false_type)

            noexcept
                    : dummy_{}
            , engaged_{false} {}

            constexpr optional_storage(tag, std::true_type)
        ...
        private:
            constexpr void swap_(std::true_type, optional_base &that)
            ...
            constexpr void swap_(std::false_type, optional_base &that)
        ...
        struct optional_base<T &> {
            ...
            constexpr explicit optional_base(in_place_t, Arg &&arg)
...
public:
    ...
    /// \pre
    requires constructible_from<T, std::initializer_list < E>
    &, Args...>)
    ...
    /// \pre
    requires (!same_as <detail::decay_t<U>, in_place_t>) AND
    (!same_as <detail::decay_t<U>, optional>)
    AND
            constructible_from<T, U>
    AND
            convertible_to<U, T>
    ...
    template(typename U)(
    /// \pre
    requires optional_should_convert<U, T>
    AND
            constructible_from<T, U const &>
    AND
    ...
    (!(
    satisfies <T, std::is_scalar> &&same_as<T, detail::decay_t<U>>
    ))
    AND
            constructible_from<T, U>
    AND
    ...
    operator=(optional<U> const &that) {
    ...
    noexcept(
            std::is_nothrow_constructible<T, std::initializer_list < E>
    &, Args...>::value)
    {
        reset();
...
namespace detail {
    namespace optional_adl {
        constexpr bool convert_bool(bool b)
    ...
    constexpr auto operator==(optional < T >
    const & x,
    optional<U> const &y
    ) //
    noexcept (noexcept(convert_bool(*x
...
template<typename T>
constexpr bool operator==(optional < T >
const & x, nullopt_t) noexcept
{
return !
x;
}
template<typename T>
constexpr bool operator==(nullopt_t, optional < T >
const & x) noexcept
{
return !
x;
...
// clang-format off
template<typename T>
auto CPP_auto_fun(swap)

(
optional<T> &x, optional<T>
&y)
(
return x.
swap(y)
...
(
return optional<detail::decay_t<T>>{
static_cast<T &&>(t)}
)
...
(
return optional<T>{
in_place, static_cast<Args &&>(args)...}
)
...
(
return optional<T>{
in_place, il, static_cast<Args &&>(args)...}
)
// clang-format on
...
namespace detail {
    ...
    struct non_propagating_cache : optional<T> {
        ...
        constexpr non_propagating_cache(nullopt_t)

        noexcept
        {}

        constexpr non_propagating_cache(non_propagating_cache const &)

        noexcept
                : optional < T > {}
                {}

        constexpr non_propagating_cache(non_propagating_cache &&that)
