...
const & that) //
noexcept(meta::and_c<
        std::is_nothrow_constructible < Ts, Us const &>::value
...>::value) //
:
common_tuple(that, meta::make_index_sequence<sizeof...(Ts)>{}
)
...
::value) //
common_tuple &operator=(std::tuple < Us...
> const & that)
noexcept(
        meta::and_c<std::is_nothrow_assignable < Ts & , Us const &>::value
...>::value)
{
(void)

tuple_transform(base(), that, element_assign_{}
...
template(typename... Us)(
/// \pre
requires std::is_assignable<detail::args
<Ts const...> &,
detail::rargs<Us...>>
::value)
...
noexcept(
        meta::and_c<std::is_nothrow_assignable
<Ts const &, Us &>::value...>::value)
{
(void)
...
detail::rargs<Us const ...>>
::value)
common_tuple const &operator=(std::tuple < Us...
> const & that)
const
noexcept(meta::and_c<
...
tuple_transform(base(), std::move(that), element_assign_{}
...
noexcept(
        meta::and_c<std::is_nothrow_constructible < Us, Ts const &>::value
...>::value)
{
return detail::to_std_tuple<Us...>(
*this, meta::make_index_sequence<sizeof...(Ts)>{
...
struct make_common_tuple_fn {
    ...
    common_tuple<bind_element_t < Args>...>

    operator()(Args &&... args) const

    noexcept(
            meta::and_c<std::is_nothrow_constructible <
                        bind_element_t < Args>, unwrap_reference_t<Args>>
    ::value...>::value)
    {
...
public:
    ...
    requires default_constructible<F>
    && default_constructible <S>)
    : std::pair <F, S>{}
    {}
    template(typename F2, typename S2)(
    /// \pre
    requires constructible_from<F, F2>
    AND constructible_from<S, S2>
    )
    ...
    template(typename F2, typename S2)(
    /// \pre
    requires constructible_from<F2, F &>
    AND constructible_from<S2, S &>
    )
    ...
    template(typename F2, typename S2)(
    /// \pre
    requires assignable_from<F &, F2 &>
    AND assignable_from<S &, S2 &>
    )
    ...
    /// \pre
    requires assignable_from<F const &, F2>
    AND assignable_from<S const &, S2>
    )
...
/// \pre
requires equality_comparable_with<F1, F2>
AND equality_comparable_with<S1, S2>
)
...
template(typename F1, typename S1, typename F2, typename S2)(
/// \pre
requires totally_ordered_with<F1, F2>
AND totally_ordered_with<S1, S2>
)
...
struct make_common_pair_fn {
    ...
    noexcept(std::is_nothrow_constructible<F, unwrap_reference_t < First>>
    ::
    value &&
            std::is_nothrow_constructible<F, unwrap_reference_t < Second>>
    ::value)
    {
