...
template<
        typename... Us,
        meta::if_<meta::and_c<
META_IS_CONSTRUCTIBLE(Us, Ts
const &)...>, int> = 0>

constexpr operator std::tuple<Us...>() const

noexcept(
        meta::strict_and<std::is_nothrow_constructible < Us, Ts const &>
...>::value)
{
return std::tuple<Us...>{
get<Is>(*this)...};
...
friend constexpr T
&
get(compressed_tuple_
& tuple) noexcept
{
return static_cast<storage<T, I, Ts...> &>(tuple).
...
{
return static_cast<storage<T, I, Ts...> const &>(tuple).
...
/// \pre
requires constructible_from<First, U>
AND constructible_from<Second, V>
)

constexpr compressed_pair(U &&u, V &&v) //
noexcept(noexcept(First((U &&) u))
&&
noexcept(Second((V &&) v)
))
: box <First, meta::size_t<0>>{
