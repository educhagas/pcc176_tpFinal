...
sentinel_t <Rng> const &get(Rng &) const
...
struct any_view_sentinel_impl<
        Rng, meta::void_<
decltype(ranges::end(std::declval < Rng
const &>()))>>
{
any_view_sentinel_impl() = default;
...
struct any_input_cursor {
    ...
    constexpr any_input_cursor(any_input_view_interface<Ref> &view)
    ...
    bool equal(any_input_cursor const &) const
...
explicit any_input_view_impl(Rng
rng)
: rng_{
std::move(rng)
}
...
any_input_view_impl(any_input_view_impl
const &) = delete;
...
{
return current_ ==
sentinel_box_t::get(rng_);
}

virtual Ref read() const
...
RANGES_NO_UNIQUE_ADDRESS Rng
rng_;
RANGES_NO_UNIQUE_ADDRESS iterator_t<Rng>
current_{
};
};
...
{
virtual void advance(std::ptrdiff_t)
= 0;
...
struct any_cursor_impl : any_cloneable_cursor_interface<Ref, Cat> {
    ...
    >);
    CPP_assert((Cat
    & category::forward) == category::forward);
...
struct any_sentinel {
    ...
    constexpr explicit any_sentinel(fully_erased_view &view)
...
public:
    ...
    CPP_ret(void)(
            /// \pre
            requires (category::bidirectional
    ...
    CPP_ret(void)(
            /// \pre
            requires (category::random_access
...
override
{
return any_cursor<Ref, Cat>{
range_box_t::get()

};
}

bool at_end(any_ref it_)

override
{
auto &it = it_.get < iterator_t < Rng >
const>();
return it ==

sentinel_box_t::get (range_box_t::get());

}

std::unique_ptr <any_cloneable_view_interface<Ref, Cat>> clone() const

override
{
return

detail::make_unique<any_view_impl> (range_box_t::get());
...
&& rng)
: ptr_{
std::make_shared <impl_t<Rng>>(views::all(static_cast<Rng &&>(rng))
)}
{
}
...
any_view(Rng
&&)
->any_view<range_reference_t < Rng>,
...
"Use any_view<Ref, category::input> instead.") =
