...
namespace ranges {
    ...
    : filter_view::remove_if_view {
    std::move(rng), not_fn(std::move(pred))
}
{
}
};
...
AND indirect_unary_predicate<Pred, iterator_t < Rng>>
AND
        view_<Rng>
AND std::is_object<Pred>::value
) //
filter_view(Rng
&&, Pred)
...
namespace views {
    ...
    struct cpp20_filter_base_fn {
        ...
        constexpr filter_view<all_t < Rng>, Pred
        >
    ...
    struct filter_base_fn : cpp20_filter_base_fn {
        ...
        operator()(Rng &&rng, Pred
        pred, Proj proj) const
        {
...
namespace cpp20 {
    ...
    AND indirect_unary_predicate<Pred, iterator_t < V>>
    AND
            view_<V>
    AND std::is_object<Pred>::value
    ) //
