...
namespace ranges {
    ...
    namespace detail {
        namespace drop_last_view {
            ...
            // Sized Bidi O(N)
            return (
            random_access_range <Rng> &&view_<Rng>
            && sized_range <Rng>) ||
            (
...
public:
    ...
    constexpr drop_last_view(Rng rng, difference_t n)
            : drop_last_view::view_adaptor(std::move(rng)), n_(n) {
        RANGES_EXPECT(n >= 0);
    }
...
bool enable_borrowed_range<drop_last_view<Rng, T>> = //
