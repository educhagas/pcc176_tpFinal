...
namespace ranges {
    ...
    namespace detail {
        ...
        constexpr bool can_sized_sentinel_()
    ...
    struct zero {
        ...
        constexpr explicit zero(T const &)
...
private box <offset_t<Const>>
    {
        ...
        default;
        constexpr
        adaptor(meta::const_if_c < Const, chunk_view_ > *cv)
        : box < offset_t<Const>>
        { 0 }
        ...
        CPP_member
                constexpr
        auto distance_to(iterator_t < CRng > const &here,
    ...
    constexpr adaptor<simple_view<Rng>()>
...
public:
    ...
    constexpr chunk_view_(Rng rng, range_difference_t <Rng> n)
            : chunk_view_::view_adaptor(detail::move(rng)), n_((RANGES_EXPECT(0 < n), n)) {}
...
private:
    ...
    private:
        ...
        private:
            ...
            constexpr iter_reference_t<iterator_t < Rng>>
            read() const
            {
                RANGES_EXPECT(!done());
                return *rng_->it();
            }
            constexpr iter_rvalue_reference_t<iterator_t < Rng>>
            move() const
            {
                RANGES_EXPECT(!done());
        ...
        public:
            ...
            constexpr explicit inner_view(chunk_view_ &view)
    ...
    public:
        ...
        constexpr explicit outer_cursor(chunk_view_ *view)

        noexcept
                : rng_{view}
                {}
        constexpr inner_view
...
bool enable_borrowed_range<chunk_view<Rng>> =
...
namespace views {
    ...
    struct chunk_base_fn {
        ...
        operator()(Rng &&rng, range_difference_t<Rng>
        n) const
        {
