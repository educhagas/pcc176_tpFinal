...
namespace ranges {
    ...
    public:
        ...
        constexpr delimit_view(Rng rng, Val value)
                : delimit_view::view_adaptor{std::move(rng)}, value_(std::move(value)) {}
    };
    ...
    bool enable_borrowed_range<delimit_view<Rng, Val>> = //
    ...
    namespace views {
        struct delimit_base_fn {
            ...
            /// \pre
            requires (!range <I_>) AND convertible_to<I_, I>
            ...
            AND semiregular<
                    Val>
            AND equality_comparable_with<Val, range_reference_t < Rng>>)
