...
namespace ranges {
    ...
    namespace detail {
        ...
        template<typename BaseIter, typename Adapt>
        constexpr always_<int,
        decltype(Adapt::read(std::declval < BaseIter
        const &>(),
...
private:
    ...
    friend
    struct adaptor_cursor;
    RANGES_NO_UNIQUE_ADDRESS compressed_pair<BaseSent, Adapt>
    data_;
...
private:
    ...
    template<typename A = Adapt, typename R = decltype(std::declval < A const &>().
    ...
    read (std::declval<BaseIter const &>())
    ...
    template<typename A = Adapt, typename =
    ...
    decltype (std::declval<adaptor_cursor const &>()
    ...
    iter_move (std::declval<BaseIter const &>())
    ...
    noexcept (noexcept(std::declval<const adaptor_cursor &>()

    .iter_move_(42)))
    ->

    decltype (std::declval<const adaptor_cursor &>()
...
public:
    ...
    /// \pre
    requires //
    (!same_as <adaptor_cursor<OtherIter, OtherAdapt>, adaptor_cursor>)
    AND
            convertible_to<OtherIter, BaseIter>
    AND
...
private:
    ...
    template<typename D>
    static constexpr adaptor_cursor_t<D>
    begin_cursor_(D
    & d)
    ...
    template<typename D>
    static constexpr adaptor_sentinel_t<D>
    end_cursor_(D
    & d)

    noexcept (noexcept(
            adaptor_sentinel_t<D> {
