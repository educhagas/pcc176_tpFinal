...
namespace ranges {
    ...
    namespace detail {
        ...
        struct slice_bounds {
            ...
            /// \pre
            requires convertible_to<F, From>
            AND convertible_to<T, To>
            )

            constexpr slice_bounds(F f, T t)
                    : from(static_cast<From>(f)), to(static_cast<To>(t)) {}
        };
        ...
        struct from_end_ {
            ...
            /// \pre
            requires integer_like_<Other>
            AND explicitly_convertible_to<Other, Int>
            )
    ...
    public:
        ...
        AND sized_range<D<True>>
        )

        constexpr bool empty() //
        noexcept (noexcept(bool(ranges::size(std::declval<D<True> &>())
        ...
        AND sized_range<D<True> const>
        )

        constexpr bool empty() const //
        noexcept (noexcept(bool(ranges::size(std::declval<D<True> const &>())
        ...
        /// \pre
        requires True
        AND (!detail::has_fixed_size_ <Cardinality>)
        AND
                forward_range<D<True>>
        )
        ...
        /// \pre
        requires True
        && detail::can_empty_<D<True>>) // clang-format off
        ...
        /// \pre
        requires True
        && detail::can_empty_<D<True> const>) // clang-format off
        ...
        constexpr detail::iter_size_t<iterator_t < D<True>>
        >
        ...
        constexpr range_reference_t<D<True>>
        ...
        /// \pre
        requires True
        AND common_range<D<True>>
        AND bidirectional_range<D<True>>
        )
        constexpr range_reference_t<D<True>>
        ...
        /// \pre
        requires True
        AND random_access_range<D<True>>
        )
        constexpr range_reference_t<D<True>>
        ...
        /// \pre
        requires True
        AND random_access_range<D<True> const>
        )
        constexpr range_reference_t<D<True> const> //
        ...
        /// \pre
        requires True
        AND contiguous_iterator<iterator_t < D<True>>
        >)
        constexpr std::add_pointer_t<range_reference_t < D<True>>
        >
        ...
        /// \pre
        requires True
        AND contiguous_iterator<iterator_t
        <D<True> const>>)
        constexpr std::add_pointer_t<range_reference_t
        <D<True> const>>
        ...
        constexpr auto //
        operator[](detail::slice_bounds<range_difference_t < D<True>>, end_fn
    ...
    private:
        ...
        !range<D<True> const>) && input_range <D<True>>)
        {
            return detail::print_rng_(sout, rng);
        }
