...
namespace ranges {
    ...
    : adjacent_remove_if_view::view_adaptor {
    detail::move(rng)
}
,
...
public:
    ...
    constexpr adaptor(adjacent_remove_if_view *rng)

    noexcept
            : rng_(rng)
            {}
    static constexpr iterator_t<Rng>
    begin(adjacent_remove_if_view
    & rng)
    {
        return *rng.begin_;
    }
    ...
    auto prev(iterator_t <Rng> &it) const //
    -> CPP_ret(void)(
            /// \pre
...
detail::non_propagating_cache<iterator_t < Rng>>
begin_;
};
