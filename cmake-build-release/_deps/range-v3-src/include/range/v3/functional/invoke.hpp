...
namespace ranges {
    ...
    "is_reference_wrapper_t is deprecated.") =
    ...
    public:
        ...
        noexcept (noexcept((invoke_fn::coerce<T>((T1 &&)

        t1, 0).*f)((Args&&) args...)))
        ->

        decltype ((invoke_fn::coerce<T>((T1 &&)

        t1, 0).*f)((Args&&) args...))
        {
            return (invoke_fn::coerce<T>((T1 && )
            t1, 0).*f)((Args &&) args...);
        }
    ...
    bool is_nothrow_invocable_v =
            detail::is_nothrow_invocable_impl_<is_invocable_v < Fn, Args...>>
    ::template apply<
            Fn, Args...>();
