...
namespace ranges {
    ...
    namespace adl_insert_detail {
        ...
        .
        insert(detail::cpp17_iterator_t<I, S>{i},
...
AND random_access_reservable<Cont>
AND
(!range <S>)) //
auto insert_impl(Cont &&cont_, P p, I i, S j, std::true_type)
->

decltype (unwrap_reference(cont_)
...
return cont.
insert(pos, C{std::move(i)}, C{std::move(j)}
);
}
...
template(typename Cont, typename I, typename Rng)(
/// \pre
requires random_access_reservable<Cont>
AND sized_range<Rng>
)
...
auto pos = insert_reserve_helper(cont, std::move(p), delta);
return cont.
insert(pos, C{ranges::begin(rng)}, C{ranges::end(rng)}
);
}
} // namespace detail
/// \endcond
