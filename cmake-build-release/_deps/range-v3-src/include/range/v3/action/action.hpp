...
namespace ranges {
    ...
    struct make_action_closure_fn {
        ...
        constexpr actions::action_closure<Fun>
    ...
    namespace actions {
        ...
        AND
                range<Rng>
        AND invocable_action_closure<ActionFn, Rng &>
        )
        ...
        return
        make_action_closure(compose(static_cast<Pipeable &&>(pipe),
...
public:
    ...
    constexpr explicit old_action_(Action a)
    ...
    /// \pre
    requires range<Rng>
    AND invocable<Action const &, Rng &, Rest...>
    )
...
template<typename ActionFn>
RANGES_INLINE_VAR constexpr
bool is_pipeable_v<actions::action_closure < ActionFn>>
=
true;
/// @}
} // namespace ranges
